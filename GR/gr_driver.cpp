...
void GRDriver::set_rk(int a) {
	assert( a >= 1);
	assert( a <= 3);
	rk = a;
}

void GRDriver::set_output_frequency_by_time(Real t) {
	int_output_freq = -1;
	time_output_freq = t;
}

void GRDriver::reset_time() {
	this->ostep_cnt = 0;
	root->set_time(0.0);
	step_cnt = 0;
}

void GRDriver::set_verbosity(int a) {
	assert( a >= 0);
	verbosity = a;
}

void GRDriver::set_output_frequency_by_step(int a) {
	assert( a >= 1);
	int_output_freq = a;
}

int GRDriver::get_rk() const {
	return rk;
}

void GRDriver::state_sums_out(FILE* fp, const State& s) {
...
void GRDriver::step_to_time(Real tmax) {
	root->set_time(0.0);
	...
	do {
		...
		step_cnt = 0;
		iters++;
	} while (iters <= GridNode::max_refine_level + 1);
	root->output("X", 0.0);
#else
...
void GRDriver::step(Real dt, bool) {
	GridNode::min_refine_density = root->state_max().d() * 1.0e-6;
...
void GRDriver::sub_step(Real dt, Real beta, int) {
	root->enforce_boundaries();
	root->clear_difs();
	root->compute_x_flux();
	root->adjust_x_flux();
...
Real GRDriver::get_time() const {
	return root->get_time();
}

void GRDriver::set_output_off() {
	int_output_freq = 0;
}
